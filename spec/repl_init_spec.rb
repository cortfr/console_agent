...
RSpec.describe ConsoleAgent::Repl, '#init_guide' do
  ...
  it 'generates guide and saves to storage' do
    ...
    saved = storage.read(ConsoleAgent::GUIDE_KEY)
  ...
  it 'passes existing guide to system prompt on re-run' do
    storage.write(ConsoleAgent::GUIDE_KEY, '# Old Guide')
  ...
  it 'does not save on empty response' do
    ...
    expect(storage.read(ConsoleAgent::GUIDE_KEY)).to be_nil
  end
